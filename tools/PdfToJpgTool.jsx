...
export default function PdfToJpgTool() {
  ...
  const convert = async () => {
    ...
    try {
      ...
      console.error('PDF to JPG error:', err?.message || err);
  ...
  const downloadOne = (page) => {
    ...
    a.href = page.url;
    a.download = `TOOLBeans-${file.name.replace('.pdf', '')}-page-${page.pageNum}.jpg`;
    a.click();
  };
    ...
    <div className="min-h-screen bg-white">
      <section className="bg-gradient-to-br from-red-50 via-white to-orange-50 border-b border-slate-100 py-12">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <nav className="flex items-center justify-center gap-2 text-xs text-slate-400 mb-5">
            ...
            <span className="text-slate-600 font-semibold">PDF to JPG</span>
          </nav>
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 mb-3">
            PDF to <span className="bg-gradient-to-r from-red-500 to-orange-500 bg-clip-text text-transparent">JPG Converter</span>
          </h1>
          <p className="text-base text-slate-500 font-light max-w-xl mx-auto leading-relaxed">
            Convert every PDF page to a high-quality JPG image. Choose resolution and quality. Runs entirely in your browser — nothing uploaded.
          </p>
          <div className="flex flex-wrap justify-center gap-8 mt-7">
            {[{v:'PDF',l:'Input'},{v:'JPG',l:'Output'},{v:'50 MB',l:'Max Size'},{v:'All Pages',l:'Supported'}].map(s => (
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select value={quality} onChange={e => setQuality(Number(e.target.value))}
                    className="w-full px-3 py-2.5 text-sm border border-slate-200 rounded-xl outline-none focus:border-red-400 bg-white text-slate-700">
                    <option value={0.7}>70% — Small file</option>
            ...
            {isConverting && (
              <div>
                <div className="flex justify-between text-xs text-slate-400 mb-1.5">
                  <span>Converting pages to JPG…</span><span>{progress}%</span>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  {pages.length > 1 && (
                    <button onClick={downloadAll} className="bg-red-500 hover:bg-red-400 text-white text-xs font-bold px-4 py-2 rounded-xl transition-all">
                      ⬇️ Download All ({pages.length} JPGs)
                    </button>
                  )}
                </div>
      ...
      <section className="max-w-4xl mx-auto px-6 pb-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            {n:'2',icon:'⚙️',t:'Pages Rendered',  d:'Each PDF page is rendered to canvas at your chosen resolution, then exported as a JPG image.'},
            {n:'3',icon:'⬇️',t:'Download Images', d:'Download individual page JPGs or all at once. Nothing is uploaded to any server.'},
          ].map(s => (
      ...
      <section className="max-w-4xl mx-auto px-6 pb-16">
        <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
          <h2 className="text-xl font-extrabold text-slate-900 mb-4">Free PDF to JPG Converter — Every Page as an Image</h2>
          <p className="text-sm text-slate-500 leading-relaxed mb-4">
            TOOLBeans PDF to JPG converter renders each page of your PDF to a high-quality JPG image using pdf.js directly in your browser. Choose from 72, 144 or 216 DPI and adjust JPEG quality. No server upload — completely private.
          </p>
          <p className="text-sm text-slate-500 leading-relaxed">
            For PNG output see <Link href="/tools/pdf-to-png" className="text-red-500 hover:underline">PDF to PNG</Link>. For text extraction see <Link href="/tools/pdf-to-text" className="text-red-500 hover:underline">PDF to Text</Link>.
          </p>
        </div>
      </section>
    </div>
