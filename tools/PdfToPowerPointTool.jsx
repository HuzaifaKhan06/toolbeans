...
export default function PdfToPowerPointTool() {
  ...
  const convert = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/convert/pdf-to-powerpoint', { method: 'POST', body: formData });
      setProgress(85);
      ...
      a.href = url; a.download = `TOOLBeans-${file.name.replace('.pdf', '')}.pptx`; a.click();
  ...
  // ── Stats with unique keys ─────────────────────────────────────────────────
  const STATS = [
    { v: 'PDF',       l: 'Input'         },
    { v: '.pptx',     l: 'Output'        },
    { v: '50 MB',     l: 'Max Size'      },
    { v: 'Editable',  l: 'Presentation'  },
  ];

  const STEPS = [
    { n:'1', icon:'📂', t:'Upload PDF',          d:'Drag and drop or click to upload any PDF up to 50 MB.'                                                                   },
    { n:'2', icon:'⚙️', t:'TOOLBeans Converts',  d:'Your file is sent to our secure server where LibreOffice converts each PDF page into an editable PowerPoint slide.'       },
    { n:'3', icon:'⬇️', t:'.pptx Downloads',     d:'Your PowerPoint file downloads automatically. The file is deleted from our server immediately.'                           },
  ];
      ...
      <section className="bg-gradient-to-br from-red-50 via-white to-orange-50 border-b border-slate-100 py-12">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <nav className="flex items-center justify-center gap-2 text-xs text-slate-400 mb-5">
            ...
            <span className="text-slate-600 font-semibold">PDF to PowerPoint</span>
          </nav>
          <div className="flex flex-wrap items-center justify-center gap-2 mb-5">
            {['Free','No Data Stored','LibreOffice Quality','.pptx Output'].map(b => (
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 mb-3">
            PDF to <span className="bg-gradient-to-r from-red-500 to-orange-500 bg-clip-text text-transparent">PowerPoint Converter</span>
          </h1>
          <p className="text-base text-slate-500 font-light max-w-xl mx-auto leading-relaxed">
            Convert PDF files to editable PowerPoint .pptx presentations. Each PDF page becomes a slide. Powered by LibreOffice on our secure server.
          </p>
      ...
      <section className="max-w-3xl mx-auto px-6 py-10">
            ...
            {status === 'done' && (
              <div className="flex flex-col gap-3">
                <div className="bg-green-50 border border-green-200 rounded-2xl p-5 text-center">
                  ...
                  <p className="font-bold text-green-800 mb-1">PowerPoint Downloaded!</p>
                  <p className="text-sm text-green-600">Saved as <strong>TOOLBeans-{file.name.replace('.pdf', '')}.pptx</strong></p>
                </div>
        ...
        <div className="mt-8 bg-amber-50 border border-amber-200 rounded-2xl p-5">
          ...
          <p className="text-xs text-slate-600 leading-relaxed">
            PDF to PowerPoint works best on single-page or clearly laid-out PDFs. Each page becomes one editable slide. Complex graphics and custom fonts may render differently in PowerPoint.
          </p>
        </div>
      </section>
        {/* 
      ...
      */}
      ...
      <section className="max-w-4xl mx-auto px-6 pb-16">
        <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
          <h2 className="text-xl font-extrabold text-slate-900 mb-4">Free PDF to PowerPoint Converter — Editable .pptx Output</h2>
          <p className="text-sm text-slate-500 leading-relaxed mb-4">
            TOOLBeans PDF to PowerPoint converter uses LibreOffice on a secure server to convert PDF files to editable .pptx presentations. Each PDF page becomes a slide. The converted file opens in Microsoft PowerPoint, Google Slides and LibreOffice Impress.
          </p>
          <p className="text-sm text-slate-500 leading-relaxed">
            Your file is uploaded securely over HTTPS, converted immediately and deleted right after your .pptx downloads. For other conversions see{' '}
            ...
            <Link href="/tools/pdf-to-excel"  className="text-red-500 hover:underline">PDF to Excel</Link>.
          </p>
        </div>
      </section>
    </div>
