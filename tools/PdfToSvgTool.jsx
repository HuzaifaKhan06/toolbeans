...
export default function PdfToSvgTool() {
  ...
  const convert = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/convert/pdf-to-svg', { method: 'POST', body: formData });
      setProgress(85);
      ...
      a.href = url; a.download = `TOOLBeans-${file.name.replace('.pdf', '')}.svg`; a.click();
  ...
  const STEPS = [
    ...
    { n:'2', icon:'⚙️', t:'TOOLBeans Converts',  d:'Your file is sent to our secure server where it is converted to a scalable SVG vector file using pdftocairo.'     },
    { n:'3', icon:'⬇️', t:'.svg Downloads',      d:'Your SVG file downloads automatically. The file is deleted from our server immediately.'                           },
  ];
    ...
    <div className="min-h-screen bg-white">
      <section className="bg-gradient-to-br from-red-50 via-white to-orange-50 border-b border-slate-100 py-12">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <nav className="flex items-center justify-center gap-2 text-xs text-slate-400 mb-5">
            ...
            <span className="text-slate-600 font-semibold">PDF to SVG</span>
          </nav>
          <div className="flex flex-wrap items-center justify-center gap-2 mb-5">
            {['Free','No Data Stored','Vector Output','.svg Format'].map(b => (
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 mb-3">
            PDF to <span className="bg-gradient-to-r from-red-500 to-orange-500 bg-clip-text text-transparent">SVG Converter</span>
          </h1>
          <p className="text-base text-slate-500 font-light max-w-xl mx-auto leading-relaxed">
            Convert PDF files to scalable SVG vector graphics. Perfect for web use and print. Powered by LibreOffice on our secure server.
          </p>
      ...
      <section className="max-w-3xl mx-auto px-6 py-10">
            ...
            {status === 'done' && (
              <div className="flex flex-col gap-3">
                <div className="bg-green-50 border border-green-200 rounded-2xl p-5 text-center">
                  ...
                  <p className="font-bold text-green-800 mb-1">SVG Downloaded!</p>
                  <p className="text-sm text-green-600">Saved as <strong>TOOLBeans-{file.name.replace('.pdf', '')}.svg</strong></p>
                </div>
        ...
        <div className="mt-8 bg-amber-50 border border-amber-200 rounded-2xl p-5">
          ...
          <p className="text-xs text-slate-600 leading-relaxed">
            PDF to SVG works best on single-page PDFs or the first page of multi-page PDFs. Vector elements like text and shapes are preserved as scalable SVG paths. Complex raster images inside PDFs will be embedded as raster in the SVG.
          </p>
        </div>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-6 pb-16">
        <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
          <h2 className="text-xl font-extrabold text-slate-900 mb-4">Free PDF to SVG Converter — Scalable Vector Output</h2>
          <p className="text-sm text-slate-500 leading-relaxed mb-4">
            TOOLBeans PDF to SVG converter converts PDF files to scalable SVG vector graphics. SVG files are resolution-independent and ideal for web use, logos and print. The output opens in browsers, Figma, Inkscape and Adobe Illustrator.
          </p>
          <p className="text-sm text-slate-500 leading-relaxed">
            Your file is uploaded securely over HTTPS, converted immediately and deleted right after your .svg downloads. For image conversions see{' '}
            <Link href="/tools/pdf-to-jpg" className="text-red-500 hover:underline">PDF to JPG</Link>,{' '}
            <Link href="/tools/pdf-to-png" className="text-red-500 hover:underline">PDF to PNG</Link> and{' '}
